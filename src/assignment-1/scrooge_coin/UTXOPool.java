...
public class UTXOPool {
    ...
    /** Creates a new empty scrooge_coin.UTXOPool */
    public UTXOPool() {
        H = new HashMap<UTXO, Transaction.Output>();
    }

    /** Creates a new scrooge_coin.UTXOPool that is a copy of {@code uPool} */
    public UTXOPool(UTXOPool uPool) {
        H = new HashMap<UTXO, Transaction.Output>(uPool.H);
    }

    /** Adds a mapping from scrooge_coin.UTXO {@code utxo} to transaction output @code{txOut} to the pool */
    public void addUTXO(UTXO utxo, Transaction.Output txOut) {
        H.put(utxo, txOut);
    }

    /** Removes the scrooge_coin.UTXO {@code utxo} from the pool */
    public void removeUTXO(UTXO utxo) {
        H.remove(utxo);
    }

    /**
     * @return the transaction output corresponding to scrooge_coin.UTXO {@code utxo}, or null if {@code utxo} is
     *         not in the pool.
     */
    ...
    /** @return true if scrooge_coin.UTXO {@code utxo} is in the pool and false otherwise */
